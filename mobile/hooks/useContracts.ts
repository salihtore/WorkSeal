/**
 * useContracts — mirrors frontend/src/hooks/useContracts.ts exactly.
 * Uses suiClient singleton instead of useSuiClient() hook.
 */
  ...
  const fetchAllContracts = useCallback(async () => {
    try {
      ...
      const filtered = allParsed.filter((contract: WorkContract) => {
        ...
        const isAssignedArb = !!(
          contract.arbitrator && addr === contract.arbitrator.toLowerCase()
        );
